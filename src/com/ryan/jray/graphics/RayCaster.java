...
public class RayCaster {
	...
	public RayObject test(double angle) {
		...
		do {
			...
			MapObject mo = map.checkPoint(this.rayPositon); // Checks if there
															// is a object and
															// return the
															// MapObject.
			if (mo != null && mo.visible) { // Check is the object is there or
				...
				if (!close && (mo.type != MapObjectType.ABERRATION)) {
					...
					if (mo.type == MapObjectType.COLOR)
						...
						return new RayObject(dist + dist2, mo, new Vector2(texX, texY), new Vector2(this.rayPositon.x, this.rayPositon.y)); // Return
																																			// a
																																			// RayObject
																																			// with
																																			// distance
			...
			dist = dist + new Vector2().distance(this.rayVelocity); // Gets the
																	...
																	// ray from
																	// the
			this.rayPositon.add(spaceWarp); // Adds the velocity to the current
											// ray position.
			// starting point.
